...
//参数设定：
#define UPOF_PIXELPDEG_X \
  160.0f  //每1角度对应的像素个数，与分辨率和焦距有关，需要调试标定。//
#define UPOF_PIXELPDEG_Y \
  160.0f  //每1角度对应的像素个数，与分辨率和焦距有关，需要调试标定。
#define UPOF_CMPPIXEL_X \
  0.00012f  //每像素对应的地面距离，与焦距和高度有关，需要调试标定。//目前粗略标定
#define UPOF_CMPPIXEL_Y \
  0.00012f  //每像素对应的地面距离，与焦距和高度有关，需要调试标定。
#define FUS_KP 2.0f
#define FUS_KI 1.0f
/**********************************************************************************************************
 ...
 *功能说明: 匿名科创光流惯性数据获取
 *参    数:
 *周期时间(s，形参)，x轴每秒弧度，y轴每秒弧度，地理x加速度(cmpss)，地理y加速度(cmpss)
 *返 回 值: 无
 **********************************************************************************************************/
  ...
  if (of_buf_update_flag != BUF_UPDATE_CNT) {
    ...
    if (XOR == of_data_buf[12]) {
      ...
      if (of_data.valid != 0xf5) {
        ...
        //
        of_data.flow_x_integral =
            (s16)(of_data_buf[4] | (of_data_buf[5] << 8));  // org_y
        //
        of_data.flow_y_integral =
            (s16)(of_data_buf[2] | (of_data_buf[3] << 8));  // org_x
      }
      //
...
static void ANO_OF_Decouple(u8 *dT_ms) {
  if (of_data.valid != 0xf5) {
    ...
    //
    of_rdf.of_vel[X] = (1000 / of_data.it_ms * of_data.flow_x_integral +
                        UPOF_PIXELPDEG_X * of_rot_d_degs[Y]);
    of_rdf.of_vel[Y] = (1000 / of_data.it_ms * of_data.flow_y_integral -
                        UPOF_PIXELPDEG_Y * of_rot_d_degs[X]);
    // quality
...
static void ANO_OF_Fusion(u8 *dT_ms, s32 ref_height_cm) {
  ...
  //
  of_rdf.gnd_vel_obs_h[X] =
      UPOF_CMPPIXEL_X * of_rdf.of_vel[X] * of_rdf.of_ref_height;
  of_rdf.gnd_vel_obs_h[Y] =
      UPOF_CMPPIXEL_Y * of_rdf.of_vel[Y] * of_rdf.of_ref_height;
  //
    ...
    case 1: {
      ...
      if (of_data.valid == 0xf5) {
        ...
        for (u8 i = 0; i < 2; i++) {
          ...
          //
          of_rdf.gnd_vel_est_w[i] +=
              (of_fus_err[i] * F_KP + of_fus_err_i[i]) * dT_s;
        }
      }
      //
