...
void FlyCtrl_Task(u8 dT_ms) {
  if (program_ctrl.cmd_state[0] != program_ctrl.cmd_state[1]) {
    ...
    FlyCtrlReset();
    //
    if (flag.rc_loss == 0 && flag.flight_mode == LOC_HOLD &&
        (switchs.of_flow_on || switchs.gps_on)) {
  ...
  switch (program_ctrl.cmd_state[0]) {
    ...
    case (0x03):  //上升
    {
      ...
      //目标时间赋值
      if (spd != 0) {
        program_ctrl.exp_process_t_ms[Z] =
            val * 1000 / LIMIT(spd, 0, fc_stv.vel_limit_z_p);
      } else {
      ...
      } else if (program_ctrl.exp_process_t_ms[Z] <
                 program_ctrl.fb_process_t_ms[Z]) {
        //
    ...
    case (0x04):  //下降
    {
      ...
      if (spd != 0) {
        program_ctrl.exp_process_t_ms[Z] =
            val * 1000 / LIMIT(spd, 0, -fc_stv.vel_limit_z_n);
      } else {
        program_ctrl.exp_process_t_ms[Z] = 0;
      }
    ...
    case (0x05):  //前进
    {
      ...
      //目标时间赋值
      if (spd != 0) {
        program_ctrl.exp_process_t_ms[X] =
            val * 1000 / LIMIT(spd, 0, fc_stv.vel_limit_xy);
      } else {
      ...
      } else if (program_ctrl.exp_process_t_ms[X] <
                 program_ctrl.fb_process_t_ms[X]) {
        //
    ...
    case (0x07):  //向左
    {
      ...
      //目标时间赋值
      if (spd != 0) {
        program_ctrl.exp_process_t_ms[Y] =
            val * 1000 / LIMIT(spd, 0, fc_stv.vel_limit_xy);
      } else {
      ...
      } else if (program_ctrl.exp_process_t_ms[Y] <
                 program_ctrl.fb_process_t_ms[Y]) {
        //
    ...
    case (0x09):  //左转
    {
      ...
      //目标时间赋值
      if (spd != 0) {
        program_ctrl.exp_process_t_ms[3] =
            val * 1000 / LIMIT(spd, 0, fc_stv.yaw_pal_limit);
      } else {
      ...
      } else if (program_ctrl.exp_process_t_ms[3] <
                 program_ctrl.fb_process_t_ms[3]) {
        //
  ...
  if (flag.unlock_sta != 0) {
    //参考方向转世界坐标（本飞控为地理坐标）
    h2w_2d_trans(program_ctrl.vel_cmps_ref, program_ctrl.ref_dir,
                 program_ctrl.vel_cmps_w);
    //世界坐标（本飞控为地理坐标）转水平航向坐标。
    w2h_2d_trans(program_ctrl.vel_cmps_w, imu_data.hx_vec,
                 program_ctrl.vel_cmps_h);
    //水平方向变化，Z不变
    program_ctrl.vel_cmps_h[Z] = program_ctrl.vel_cmps_w[Z] =
        program_ctrl.vel_cmps_ref[Z];
  } else {
    //记录机头指向为参考方向
