...
void IMU_Update_Task(u8 dT_ms) {
  ...
  //				imu_state.obs_en = 1;
  //				imu_data.obs_acc_w[X] =
  //flow_decoupling.w_ref_acc[X]; 				imu_data.obs_acc_w[Y] =
  //flow_decoupling.w_ref_acc[Y];

  /*姿态计算，更新，融合*/
  IMU_update(dT_ms * 1e-3f, &imu_state, sensor.Gyro_rad, sensor.Acc_cmss,
             mag.val, &imu_data);  // x3_dT_1[2] * 0.000001f
  //////////////////////////////////////////////////////////////////////
}
...
void Mag_Update_Task(u8 dT_ms) {
  ...
  Mag_Data_Deal_Task(dT_ms, mag_val, imu_data.z_vec[Z], sensor.Gyro_deg[X],
                     sensor.Gyro_deg[Z]);
}

s32 baro_height, baro_h_offset, ref_height_get_1, ref_height_get_2,
    ref_height_used;
...
// void Baro_Get_Task()
//{
////			ref_height_get += LIMIT((s32)user_spl0601_get() -
///ref_height_get,-20,20 ); /	baro_height =(s32)user_spl0601_get();
//}
...
void WCZ_Fus_Task(u8 dT_ms) {
  ...
  {
    ref_height_get_1 = baro_height - baro_h_offset + baro_fix +
                       tof2baro_offset;  //气压计相对高度，切换点跟随TOF
                                         // baro_offset_ok = 0;
  } else {
    if (baro_offset_ok == 0) {
  ...
  if ((sens_hd_check.tof_ok || sens_hd_check.of_ok) &&
      baro_offset_ok)  // TOF或者OF硬件正常，且气压计记录相对值以后
  {
    if (switchs.tof_on || switchs.of_tof_on)  // TOF数据有效
    {
      ...
      if (tof_offset_ok == 1) {
        ref_height_get_2 =
            ref_tof_height + baro2tof_offset;  // TOF参考高度，切换点跟随气压计

        ref_height_used = ref_height_get_2;

        tof2baro_offset +=
            0.5f *
            ((ref_height_get_2 - ref_height_get_1) -
             tof2baro_offset);  //记录气压计切换点，气压计波动大，稍微滤波一下
